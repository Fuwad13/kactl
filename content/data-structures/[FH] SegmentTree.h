 ...
 * License: CC0
 * Source: Amar Matha
 * Description: Recursive Segment Tree. Bounds are inclusive. 
 * Can be used for various operations , just need to change the merge function.
 * Usage: segment_tree<int> st(arr, [](int a, int b){return a+b;}, 0);
 * Time: O(\log N)
...
struct segment_tree{
    ...
    void set(int node, int b, int e, int ind, T val){
        ...
        int mid = (b+e)>>1;
        set(2*node, b, mid, ind, val); set(2*node+1, mid+1, e, ind, val);
        stree[node] = merge(stree[2*node],stree[2*node+1]);
    ...
    void update(int node, int b, int e, int ind, T val){
        ...
        int mid = (b+e)>>1;
        update(2*node, b, mid, ind, val); update(2*node+1, mid+1, e, ind, val);
        stree[node] = merge(stree[2*node],stree[2*node+1]);
    }
